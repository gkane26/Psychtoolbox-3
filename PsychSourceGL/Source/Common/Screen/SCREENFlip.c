...
PsychError SCREENFlip(void)
{
    ...
    "Please note that the semantic and accuracy / reliability of the returned timestamps may differ if special display backends "
    "or display devices are in use. With the optional Vulkan display backend for special HDR, VRR or high bit depth display "
    "modes, the driver may only return estimated stimulus onset time, as an identical value in both 'StimulusOnsetTime' and "
    "in 'VBLTimestamp'. Especially with special Virtual Reality (VR), Augmented Reality (AR) and Mixed Reality (MR) displays, "
    "commonly summarized under the name eXtended Reality (XR), usually only estimated stimulus onset time is returned, again in "
    "both 'StimulusOnsetTime' and in 'VBLTimestamp'. The robustness, trustworthiness, precision and accuracy of both returned "
    "stimulus onset timestamps and of adherence to the requested stimulus onset time \"when\" will vary greatly depending on the "
    "specific operating system, Psychtoolbox backend driver, vendor provided runtime driver and hardware. Also, precision often "
    "comes at the price of increased latency or lower performance or presentation framerates, depending on the tradeoffs you "
    "choose. Read the sections about timing, timestamping and precision vs. performance tradeoffs in 'help PsychVRHMD' carefully "
    "to understand what to expect and how to tune operations for your specific experimental needs if you use a VR/AR/MR/XR device.\n"
    "'Beampos' is the position of the monitor scanning beam when the time measurement was taken (useful for correctness tests). "
    "This value is only available on a subset of operating systems, graphics cards and display configurations. A constant value of "
    "-1 or zero means unsupported.\n"
    ...
    "'Missed' indicates if the requested presentation deadline for your stimulus has been missed. A negative value means that "
    "deadlines have been satisfied. Positive values indicate a deadline-miss. The automatic detection of deadline-miss is not fool-proof "
    "- it can report false positives and also false negatives, although it should work fairly well with most experimental setups. "
    "If you are picky about timing, please use the provided timestamps or additional methods to exercise your own tests. In case of "
    "use of special display backends like Vulkan for VRR, HDR or high bit depth display modes, or when used with VR/AR/MR/XR displays "
    "the 'Missed' value is meaningless or unreliable at the moment. It only makes sense in standard display modes on standard hardware "
    "at the moment.\n";
